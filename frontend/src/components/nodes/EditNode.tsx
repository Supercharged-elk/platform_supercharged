...
export function EditNode({ id }: NodeProps) {
  ...
  return (
    <div className="bg-neutral-900 border border-neutral-700 rounded-xl p-3 w-56 relative">
      ...
      <div className="text-xs font-semibold text-neutral-400 mb-2 uppercase tracking-wide">Edit Image</div>

      {state.status === "running" && (
        <div className="mb-2">
          <div className="w-full bg-neutral-800 rounded-full h-1.5">
            <div className="bg-purple-500 h-1.5 rounded-full" style={{ width: `${state.progress ?? 0}%`, transition: (state.progress ?? 0) > 0 ? 'width 0.7s ease' : 'none' }} />
          </div>
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-purple-600 hover:bg-purple-500 disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm rounded-lg transition"
      >
        {state.status === "running" ? <X size={14} /> : <Wand2 size={14} />}
        {state.status === "running" ? "Cancel" : "Edit"}
      </button>
