          ...
          <div key={vid.id} className="bg-neutral-800 rounded-xl border border-neutral-700 overflow-hidden">
            ...
            <div className="aspect-square bg-neutral-900 flex items-center justify-center relative">
              {vid.status === "generating" && (
                <GeneratingState message="Animating…" subMessage="Up to 3 minutes" />
              )}
              ...
              {vid.status === "idle" && (
                <img
                  src={base64ToDataUrl(vid.imageBase64, vid.mimeType)}
                  alt="Illustration"
              ...
              {vid.status === "error" && (
                <div className="p-3 w-full">
                  <ErrorBlock
                    message={vid.error ?? "Generation failed"}
            ...
            <div className="p-3 space-y-2">
              ...
              {vid.status === "idle" && (
                <button
                  type="button"
                  onClick={() => void generateVideo(vid.id, localPrompts[vid.id])}
                  className="w-full py-1.5 text-xs rounded-lg bg-violet-600 hover:bg-violet-500 text-white font-medium transition"
                >
                  Generate Video
                </button>
              )}
      ...
      <div className="flex items-center justify-between pt-2">
        <div className="space-y-1">
          ...
          <p className="text-xs text-neutral-600">
            Going back and re-confirming prompts will clear generated videos.
          </p>
        </div>
      </div>
    </div>
